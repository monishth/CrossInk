...
| 4.2 | **Ground truth is generated by the KOReader emulator**, not taken from sidecars | Both sidecars on this machine carry the *identical* cover-page xpointer and zero annotations. The emulator at `koreader/koreader-emulator-x86_64-pc-linux-gnu-debug/` is built and runs, and exposes `getPageXPointer()` | 🟡 | Falls back to a synthesized corpus, which proves much less |
