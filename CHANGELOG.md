...
- Highlights and bookmarks now sync to the right place in the book. Both directions were mapping positions with CrossInk's older position code, which only understands the layout of pre-2020 KOReader files — against anything a current KOReader wrote it resolved nothing, so highlights arriving from another device all landed at the start of the book, and highlights sent from this one named a whole chapter rather than a place in it. A highlight that names a whole chapter is also indistinguishable from every other highlight in that chapter, which is what made them collide. Each highlight and bookmark now records the position it was made at, and sync uses the same mapping that reading-position sync already used. Highlights saved before this update keep working as they did; new ones carry the better anchor.
- Reading position now syncs in both directions. Position was only ever uploaded: a position fetched from BookOrbit was decoded, checked, and then discarded, and the "approximate position" message reported a jump that never happened. Sync now moves the reader to the newer position when another device has read further, and says so. If your own device has read more recently, it uploads instead — whichever side read last wins, decided from actual reading activity rather than from when a device last synced. A newer position that arrives without an exact location leaves both sides untouched rather than overwriting it.
